...
public class CreativeJsonJacksonTest {
	...
	public void testTreeModelExample() throws JsonParseException, JsonMappingException, IOException {
		File creativeJsonFile=new File(getClass().getClassLoader().getResource("creative.json").getFile());
		...
		// basic assertion
		assertEquals("assertion for adformat value is 'Banner'", "Banner", rootNode.path("adFormat").getTextValue());
		assertEquals("assertion for multiClicks element count", 3, multiClicksNode.size());
		
		// multiClicks assertion data setting
		List<String> verifyClickUrlList = new ArrayList<String>() {
			{
				add("www.daum.net");
				add("www.naver.net");
				add("www.nate.com");
		...
		List<String> verifyCreativeIdList = new ArrayList<String>() {
			{
				add("c01");
				add("c02");
				add("c03");
		...
		List<String> verifyPositionList = new ArrayList<String>() {
			{
				add("left");
				add("center");
				add("right");
