 ...
 *
 * <p>By default, this test runs with HtmlUnit, but this behavior can be changed by having a
 * property file named "test.properties" in the HtmlUnit root directory. Sample:
 *
 * <pre>
 ...
 *
 * <ul>
 *   <li>browsers: is a comma separated list contains any combination of "hu" (for HtmlUnit), "ff2",
 *       "ff3", "ie6", "ie7", "ie8", which will be used to driver real browsers, note that you can't
 *       define more than one IE as there is no standard way to have multiple IEs on the same
 *       machine
 *   <li>ff2.bin: is the location of the FF2 binary, in Windows use double back-slashes
 *   <li>ff3.bin: is the location of the FF3 binary, in Windows use double back-slashes
 * </ul>
 *
...
public abstract class WebDriverTestCase extends WebTestCase {
  ...
  private static Map<BrowserVersion, WebDriver> WEB_DRIVERS_ =
      new HashMap<BrowserVersion, WebDriver>();
   ...
   * @throws Exception if something goes wrong
   */
  protected final void loadPageWithHtml(final String html) throws Exception {
    loadPageWithHtml(html, URL_FIRST);
  }
   ...
   * @throws Exception if something goes wrong
   */
  protected final void loadPageWithHtml(final String html, final URL url) throws Exception {
    final MockWebConnection mockWebConnection = getMockWebConnection();
    mockWebConnection.setResponse(url, html);
    startWebServer(mockWebConnection);
  }
  ...
  /**
   * Loads the provided URL serving responses from {@link #getMockWebConnection()} and verifies that
   * the captured alerts are correct.
   *
   * @param url the URL to use to load the page
   * @return the web driver
   * @throws Exception if something goes wrong
  ...
  /**
   * Gets the alerts collected by the driver. Note: it currently works only if no new page has been
  ...
  protected List<String> getCollectedAlerts(final WebDriver driver) throws Exception {
    ...
    if (driver instanceof HtmlUnitDriver) {
      ...
      final String jsonResult =
          (String)
              jsExecutor.executeScript(getJSON() + ";return JSON.stringify(top.__huCatchedAlerts)");
      if (jsonResult != null) {
  ...
  /**
   * Release resources but DON'T close the browser if we are running with a real browser. Note that
   * HtmlUnitDriver instances are not cached.
   */
  @After
  @Override
...
/**
 * As HtmlUnit didn't generate the right events, WebDriver did it for us, but now that we do it
 * correctly, WebDriver shouldn't do it anymore
