...
public class DeserializeJsonWithJacksonExample2 {
	public String SimpleCollectionDeserializationAndSerialization(String runType)
			throws IOException {

		File jsonFile = new File(getClass().getResource("input_2.json")
				.getFile());
		...
		if ("TypeReference".equals(runType)) {
			animals = mapper.readValue(jsonFile,
					new TypeReference<Collection<AnimalExample2>>() {
					});
			...
			animals = mapper.readValue(
					...
					TypeFactory.defaultInstance().constructParametricType(
							Collection.class, AnimalExample2.class));
		}
