...
/**
 * The custom runner <code>BrowserRunner</code> implements browser parameterized tests. When running
 * a test class, instances are created for the cross-product of the test methods and {@link
 * BrowserVersion}s.
 *
 * <p>For example, write:
 *
...
public class BrowserRunner extends Suite {
    ...
    if (BrowserVersionClassRunner.containsTestMethods(klass)) {
      if (WebDriverTestCase.class.isAssignableFrom(klass)) {
        ...
        if (property.contains("hu")) {
          runners_.add(
              new BrowserVersionClassRunner(klass, BrowserVersion.INTERNET_EXPLORER_6, false));
          runners_.add(
              new BrowserVersionClassRunner(klass, BrowserVersion.INTERNET_EXPLORER_7, false));
          runners_.add(
              new BrowserVersionClassRunner(klass, BrowserVersion.INTERNET_EXPLORER_8, false));
          runners_.add(new BrowserVersionClassRunner(klass, BrowserVersion.FIREFOX_3, false));
        }
        if (property.contains("ie6")) {
          runners_.add(
              new BrowserVersionClassRunner(klass, BrowserVersion.INTERNET_EXPLORER_6, true));
        }
        if (property.contains("ie7")) {
          runners_.add(
              new BrowserVersionClassRunner(klass, BrowserVersion.INTERNET_EXPLORER_7, true));
        }
        if (property.contains("ie8")) {
          runners_.add(
              new BrowserVersionClassRunner(klass, BrowserVersion.INTERNET_EXPLORER_8, true));
        }
  ...
  /**
   * The only {@link Browser}s that are expected to succeed, default value is all. For example, if
   * you use <tt>@Browsers(Browser.INTERNET_EXPLORER_6)</tt> that means only IE6 is expected to
   * succeed, but IE7, FF2 and FF3 should fail.
   */
  ...
  public static @interface Browsers {
    ...
    Browser[] value() default {Browser.IE, Browser.FF};
  }
