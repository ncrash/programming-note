...
public class UUIDTimerTest extends TestCase {
   ...
   * tests ***********************************************************************
   */
  /** Test of UUIDTimer(SecureRandom) constructor, of class org.safehaus.uuid.UUIDTimer. */
  public void testSecureRandomUUIDTimerConstructor() {
    ...
    assertArrayNotEqual(test_array, new byte[UUID_TIMER_ARRAY_LENGTH], UUID_TIMER_ARRAY_LENGTH);
  }
  /**
  ...
  public void testGetTimestamp() {
    ...
    for (int i = 0; i < EXTRA_DATA_LENGTH; ++i) {
      assertEquals(
          "test_array element was corrupted", (byte) 'x', test_array[i + UUID_TIMER_ARRAY_LENGTH]);
    }
    ...
    Long[] uuid_timer_array_of_longs =
        convertArrayOfByteArraysToArrayOfLongs(array_of_uuid_timer_byte_arrays);
    ...
    //        checkUUIDTimerLongArrayForCorrectCreationTime(uuid_timer_array_of_longs, start_time,
    // end_time);
  }
   ...
   * ***********************************************************************
   */
  private Long[] convertArrayOfByteArraysToArrayOfLongs(byte[][] uuidTimerArrayOfByteArrays) {
  ...
  private void checkUUIDTimerLongArrayForCorrectOrdering(Long[] uuidTimerLongArray) {
    ...
    for (int i = 0; i < sorted_arrays_length; i++) {
      assertTrue(
          "Reverse order check on uuid timer arrays failed"
              ...
              + i
              + ": "
              + uuidTimerLongArray[i].longValue()
              + " does not equal "
              + uuid_timer_sorted_arrays[sorted_arrays_length - (1 + i)].longValue(),
          uuidTimerLongArray[i].equals(uuid_timer_sorted_arrays[sorted_arrays_length - (1 + i)]));
    }
  ...
  private void checkUUIDTimerLongArrayForUniqueness(Long[] uuidTimerLongArray) {
    ...
    for (int i = 0; i < uuidTimerLongArray.length; i++) {
      assertTrue("Uniqueness test failed on insert into HashSet", set.add(uuidTimerLongArray[i]));
      assertFalse(
  ...
  private void checkUUIDTimerLongArrayForCorrectCreationTime(
      Long[] uuidTimerLongArray, long startTime, long endTime) {
    ...
    for (int i = 0; i < uuidTimerLongArray.length; i++) {
      ...
      assertTrue(
          ...
          startTime <= uuid_time);
      assertTrue(
          "UUID timestamp: " + uuid_time + " was not before the end time: " + endTime,
          uuid_time <= endTime);
    }
  }

  private void checkUUIDTimerLongArrayForNonNullTimes(Long[] uuidTimerLongArray) {
    for (int i = 0; i < uuidTimerLongArray.length; i++) {
      assertFalse("Timer Long was null", 0 == uuidTimerLongArray[i].longValue());
    }
  }

  private void assertArrayNotEqual(byte[] array1, byte[] array2, int length) {
    assertTrue("array1 was not equal or longer then length", array1.length >= length);
    assertTrue("array2 was not equal or longer then length", array2.length >= length);
