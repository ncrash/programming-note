...
public class TagURITest extends TestCase {
  ...
  public void testToString() {
    ...
    for (int i = 0; i < 4; ++i) {
      for (int j = 0; j < 4; ++j) {
        ...
        assertEquals(
            "Expected string did not match generated toString()", expected, tag_uri.toString());
      }
    }
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      for (int j = 0; j < 4; ++j) {
        TagURI tag_uri = new TagURI(AUTHORITIES[i], IDS[j], CALENDAR);
        String expected =
            "tag:" + AUTHORITIES[i] + "," + CALENDAR.get(Calendar.YEAR) + ":" + IDS[j];
        assertEquals(
            "Expected string did not match generated toString()", expected, tag_uri.toString());
      }
    }
  ...
  public void testEquals() {
    ...
    assertFalse("x.equals(non_TagURI_object) didn't return false", x.equals(new Object()));
