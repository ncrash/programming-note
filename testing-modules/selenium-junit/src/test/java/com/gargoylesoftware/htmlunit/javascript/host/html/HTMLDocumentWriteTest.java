...
public class HTMLDocumentWriteTest extends WebDriverTestCase {
  ...
  /**
   * <a
   * href="https://sourceforge.net/tracker/?func=detail&aid=2855731&group_id=47038&atid=448266">Bug
   * 2855731</a>.
   *
  ...
  /**
   * Caused infinite loop at some point of 2.6 snapshot. See <a
   * href="http://sourceforge.net/support/tracker.php?aid=2824922">Bug 2824922</a>
   *
   * @throws Exception if the test fails
   */
  @Test
  ...
  /**
   * Verifies that document.write() sends content to the correct destination (always somewhere in
   * the body).
   *
   * @throws Exception if an error occurs
   */
  ...
  /**
   * Verifies that document.write() sends content to the correct destination (always somewhere in
   * the body), and that if a synthetic temporary body needs to be created, the attributes of the
   * real body are eventually used once the body is parsed.
   *
   * @throws Exception if an error occurs
   */
  @Test
  ...
  /**
   * Verifies that document.write() sends content to the correct destination (always somewhere in
   * the body), and that script elements written to the document are executed in the correct order.
   *
   * @throws Exception if an error occurs
   */
  @Test
  ...
  /**
   * Test for bug 1950462: calling document.write inside a function (after assigning document.write
   * to a local variable) tries to invoke document.write on the prototype document instance, rather
   * than the actual document host object. This leads to an {@link IllegalStateException} (DomNode
  ...
  /**
   * Verifies that scripts added to the document via document.write(...) don't execute until the
   * current script finishes executing; bug found at <a href="http://code.google.com/apis/maps/">the
   * Google Maps API site</a>.
   *
   * @throws Exception if an error occurs
   */
  @Test
  ...
  /**
   * Regression test for bug 2884585. As of HtmlUnit-2.7-SNAPSHOT 17.01.2010 <script src="..."...
   * written by document.write was not loaded and executed after the </script> when the page was
   * loaded as result of a click.
   *
   * @throws Exception if the test fails
   */
  @Test
  ...
  /**
   * Partial regression test for bug 2921851: use opener URL as base URL for resolution of relative
   * URLs in document.write.
   *
   * @throws Exception if the test fails
   */
  @Test
  public void urlResolutionInWriteForm() throws Exception {
    ...
    assertEquals(
        new URL(getDefaultUrl(), "foo"),
        getMockWebConnection().getLastWebRequestSettings().getUrl());
  }
  ...
  @NotYetImplemented
  @Alerts(
    FF = {"<form></form>", "[object HTMLFormElement]"},
    IE = {"<FORM></FORM>", "[object]"}
  )
  ...
  /**
   * Partial regression test for bug 2921851: the document returned by <tt>window.document</tt>
   * should be proxied (i.e. "live").
   *
   * @throws Exception if an error occurs
   */
  @Ignore
